...
Loader {
    ...
    Component.onCompleted: {
        if (source != "") {
            ...
            console.log(typeof sourceComponent)
            delayed_source_component = sourceComponent
            sourceComponent = (Date.now() >= begin_at && Date.now() < end_at) ? delayed_source_component : undefined
        }
    }
    Timer {
        interval: parent.begin_at - Date.now()
        onTriggered: { 
            if (parent.delayed_source != "")
                parent.source = parent.delayed_source
            else if (typeof parent.delayed_source_component !== "undefined")
                parent.sourceComponent = parent.delayed_source_component
        }
